...
import { copyDir, isMain, log, OUT, REGISTRY, SHARED, SITE_DIR, TOOLS, toolDirs } from "./lib.mjs";

export function assemble() {
  if (!fs.existsSync(path.join(OUT, "index.html"))) throw new Error("site/out/index.html missing: run `next build` in site/ first");
  ...
  log.ok(`assembled site/out/: site + ${dirs.length} tool(s) [${dirs.join(", ") || "none"}] + shared/`);
}

export function build({ quiet = false } = {}) {
  gen({ quiet });
  log.info("next build …");
  execSync("npx next build", { cwd: SITE_DIR, stdio: quiet ? "pipe" : "inherit" });
  assemble();
}
