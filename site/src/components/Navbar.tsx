...
import { Globe, Linkedin, Mail, Moon, Sun } from "lucide-react";
import { useLabStore } from "@/store/useLabStore";
import { dictionary } from "@/lib/dictionary";
import { LAB_CONFIG, isSet } from "@/lib/config";

export function Navbar() {
  const { lang, setLang, theme, setTheme } = useLabStore();
  const t = dictionary[lang];

  return (
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-rule bg-bg">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="hidden sm:flex items-center gap-3 ml-1 pl-3 border-l border-rule">
